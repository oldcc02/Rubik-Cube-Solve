...
async def get_file_list(file_dir_path: str) -> list:
    ...
    """
    res = []
    for img_name in await aio_os.listdir(file_dir_path):
        sub = await aio_os.path.splitext(img_name)
        if sub[-1] in img_type:
            res.append(await aio_os.path.join(file_dir_path, img_name))
...
async def init_dir(dir_name: str = "imgs"):
    ...
    pre_path = await aio_os.path.dirname(await aio_os.getcwd())
    img_root_path = await aio_os.path.join(pre_path, dir_name)
    for c in color_class:
        color_class_dir = await aio_os.path.join(img_root_path, c)
        if await aio_os.path.exists(color_class_dir):
            continue
